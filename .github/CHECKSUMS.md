...
`full_output_checksums.txt` provides md5 checksums for HH current output on my local machine. This program enables users to validate their output compared to my own. There's no reason why your output should differ from my own, but on the off chance that it does, [please open up a new issue](https://github.com/IcePanorama/HarvesterHarvester/issues).
