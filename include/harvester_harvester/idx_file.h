 ...
 *  index files that we're ignoring here. Between the `path` and `extent_loc`
 *  data fields, there's some data, but I can't for the life of me seem to
 *  figure out what it's used for. It could be random garbage generated by
 *  whatever program created these files, they could be file flags, or perhaps
 *  even some sort of checksum; I'm not sure. If you have any idea what this
 *  data might be used for, feel free to open an issue/pull request modifying
 *  this project as needed! Please see the `harvester_harvester/idx_file.c`
 *  file for more details.
 *
 *  TODO: Include example of some unused data an include it somewhere in the
 *  project's .github folder to make it easier for anybody to take a look at
 *  it.
 *
 ...
 *  immediately.
 *
 *  Param:  i  a pointer to an _HHIndexFile_t
 */
